...
csv_names = ["../02_form_vectors/01_CSC_vectors/CSC_sent_vector.csv", "../02_form_vectors/01_CSC_vectors/CSC_word_vector.csv", "../02_form_vectors/01_CSC_vectors/CSC_char_vector.csv", "../02_form_vectors/01_CSC_vectors/CSC_punct_vector.csv"]
readers = [csv.reader(open(r, 'r')) for r in csv_names]
writer = csv.writer(open('CSC_main_vectors.csv', 'w'))
for row in zip(*readers):
    writer.writerow(list(itertools.chain.from_iterable(row)))
csc_form = pd.read_csv('CSC_main_vectors.csv', sep=',', header=1)
csc_content = pd.read_csv("../01_content_vectors/content_wo.csv", sep=',', header=0)
data_matrix = pd.concat([pd.DataFrame(csc_form), pd.DataFrame(csc_content)], axis=1)
data_matrix.to_csv('CSC_main_vectors.csv', index=False, delimiter=',')


# FOR SAD: merging data vectors for test, stored in 'SAD_main_vectors.csv'
csv_names = ["../02_form_vectors/02_SAD_vectors/SAD_sent_vector.csv", "../02_form_vectors/02_SAD_vectors/SAD_word_vector.csv", "../02_form_vectors/02_SAD_vectors/SAD_char_vector.csv", "../02_form_vectors/02_SAD_vectors/SAD_punct_vector.csv"]
readers = [csv.reader(open(r, 'r')) for r in csv_names]
writer = csv.writer(open('SAD_main_vectors.csv', 'w'))
for row in zip(*readers):
    writer.writerow(list(itertools.chain.from_iterable(row)))
sad_form = pd.read_csv('SAD_main_vectors.csv', sep=',', header=1)
sad_content = pd.read_csv("../01_content_vectors/test_content_wo.csv", sep=',', header=0)
data_matrix = pd.concat([pd.DataFrame(sad_form), pd.DataFrame(sad_content)], axis=1)
data_matrix.to_csv('SAD_main_vectors.csv', index=False, delimiter=',')


# input data FOR CSC
clean_data = pd.read_csv('CSC_main_vectors.csv', sep=',', header=0)
...
clean_test_data = pd.read_csv('SAD_main_vectors.csv', sep=',', header=0)
