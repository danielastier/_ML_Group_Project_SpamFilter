...
# input data
clean_data = pd.read_csv('CSC_content.csv', sep=',', header=0)
...
# input test data
clean_test_data = pd.read_csv('SAD_content.csv', sep=',', header=0)
...
print("################### TEST DATA: Logistic Regression C=1000000 (10-fold cross validation) ###################")
...
# set variables
#TODO: calculate max value for dataframe, insert in max_features: int(dataframe.values.max())+1
...
for i, (train, test) in enumerate(kfold_cnn):
    ...
    cnn_cv.add(MaxPooling1D(pool_length=cnn_cv.output_shape[1]))
    cnn_cv.add(Flatten())
...
print("################### TEST DATA: CNN (10-fold cross validation) ###################")
print("labels", len(test_labels))
print("data_features", test_data_features.shape)
...
for i, (train_test, test_test) in enumerate(kfold_cnn_test):
    ...
    cnn_cv_test.add(MaxPooling1D(pool_length=cnn_cv.output_shape[1]))
